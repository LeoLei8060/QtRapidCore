    ...
    ui(new Ui::MainWindow)
{
    ...
    // 项目类型变更时更新UI状态
    connect(ui->projectTypeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), 
            this, &MainWindow::on_projectTypeComboBox_currentIndexChanged);
...
{
    // 当选择QQuick2ApplicationWindow时，禁用Form文件选项
    bool isQml = (index == 2); // QQuick2ApplicationWindow
    ui->useFormCheckBox->setEnabled(!isQml);
    if (isQml) {
        ui->useFormCheckBox->setChecked(false);
    }
...
void MainWindow::updateFileNamePreview()
{
    ...
    bool useForm = ui->useFormCheckBox->isChecked() && projectType != 2; // QQuick2ApplicationWindow不使用Form
    
    if (projectType == 2) { // QQuick2ApplicationWindow
        ui->formFilePreviewLabel->setText(projectName + ".qml");
    } else {
...
QMap<QString, QVariant> MainWindow::getProjectConfig()
{
    ...
    switch (projectTypeIndex) {
        case 0: projectType = "QWidget"; break;
        case 1: projectType = "QMainWindow"; break;
        case 2: projectType = "QQuick2ApplicationWindow"; break;
        default: projectType = "QWidget";
    }
