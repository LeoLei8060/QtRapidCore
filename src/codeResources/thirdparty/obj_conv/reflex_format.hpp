...
static QPair<bool, std::string> json_to_obj(const std::string &str, _type &obj, bool isfile = false, Args &&...args)
{
    JsonReader reader(str, isfile);
    return qMakePair(reader.convert(nullptr, obj, std::forward<Args>(args)...), reader.get_json());
}
//"indentCount = 0"  show json format. "indentChar" show split character
template<typename _type>
static std::string obj_to_json(const _type       &obj,
...
template<typename _type>
static bool obj_to_save_xml_file(const char        *filename,
                                 const _type       &data,
                                 const std::string &root = "",
...
#define STRUCT_FUNC_TOX_BEGIN \
    ...
    {
//bind variable name
#define STRUCT_ACT_TOX_O(M) obj.convert(#M, this->M);
//alias variable name
#define STRUCT_ACT_TOX_A(M, A_NAME) obj.convert(reflextraits::alias_name_conversion(#M, A_NAME).c_str(), this->M);
//inherited class variable name
#define STRUCT_ACT_TOX_I(CLASS) CLASS::obj_to_struct(obj);
...
#define STRUCT_FUNC_TOS_BEGIN \
    ...
    {
//bind variable name
#define STRUCT_ACT_TOS_O(M) obj.convert(#M, this->M);
//alias variable name
#define STRUCT_ACT_TOS_A(M, A_NAME) obj.convert(reflextraits::alias_name_conversion(#M, A_NAME).c_str(), this->M);
//inherited class variable name
#define STRUCT_ACT_TOS_I(CLASS) CLASS::struct_to_obj(obj, root);
#define STRUCT_FUNC_TOS_END     }
/******************************************macro epand param******************************************/
#define STRUCT_COUNT(LEVEL, \
                     ACTION, \
                     _99, \
                     _98, \
                     _97, \
...
#define STRUCT_N(LEVEL, ACTION, ...) \
    STRUCT_EXPAND(STRUCT_COUNT(LEVEL, \
                               ACTION, \
                               __VA_ARGS__, \
                               _99, \
                               _98, \
...
#define REFLEX_BIND(...) \
    STRUCT_FUNC_COMMON \
    STRUCT_FUNC_TOX_BEGIN STRUCT_N(STRUCT_L1, STRUCT_L1_TOX, __VA_ARGS__) \
    STRUCT_FUNC_TOX_END \
    STRUCT_FUNC_TOS_BEGIN STRUCT_N(STRUCT_L1, STRUCT_L1_TOS, __VA_ARGS__) STRUCT_FUNC_TOS_END

#define REFLEX_BIND_VOID(...) \
    STRUCT_FUNC_COMMON \
    STRUCT_FUNC_TOX_BEGIN STRUCT_FUNC_TOX_END STRUCT_FUNC_TOS_BEGIN STRUCT_FUNC_TOS_END
